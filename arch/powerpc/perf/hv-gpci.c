...
static void add_sysinfo_interface_files(void)
{
	...
	/* Get struct device_attribute for a given counter request value */
...
out:
	/*
	 * Add sysinfo interface attributes, only if the hcall passed for all
	 * counter request values. Otherwise, free the added device attribute array entries.
	 */
	if (i > 0) {
		while (i >= 0) {
			kfree(attr[i]);
