			...
			struct device_attribute *attr, char *buf)
{
	 ...
	 * processor index, hence passing the value 0.
	 */
	ret = systeminfo_gpci_request(0xA0, 0, 0, buf, &n, arg);
	...
	while (be32_to_cpu(arg->params.detail_rc) == 0x1b) {
		 ...
		 * in the counter_value buffer as an element, use the value in the
		...
		ret = systeminfo_gpci_request(0xA0, starting_index, secondary_index, buf, &n, arg);
