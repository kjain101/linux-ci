			...
			size_t *n, struct hv_gpci_request_buffer *arg)
{
	...
	if (ret && (ret != H_PARAMETER)) {
		pr_info("Unknown error while retrieving system information, Err:%ld\n", ret);
		return -EINVAL;
	}
			...
			struct device_attribute *attr, char *buf)
{
	 ...
	 * which implies can't accommodate all information, and a partial buffer
	 * returned. To handle that, we need to take subsequent requests
	 * with greater secondary index to retrieve additional (missing) data.
	 * Below loop do subsequent hcalls with greater secondary index and add it
	 * to buffer util we get all the information.
	 */
	while (ret == H_PARAMETER) {
		int returned_values = be16_to_cpu(arg->params.returned_values);
		...
		/*
		 * Since the starting index and secondary index type is included
		 * in the counter_value buffer as an element, parse the value in the
		 * last array element as subsequent starting index, and use the value
		 * in the last array element plus 1 as subsequent secondary index.
							...
							char *buf)
{
	...
	/*
	 * ret value as 'H_PARAMETER' implies that buffer size
	 * which implies can't accommodate all information, and a partial buffer
	 * returned. To handle that, we need to take subsequent requests
	 * with greater starting index to retrieve additional (missing) data.
	 * Below loop do subsequent hcalls with greater starting index and add it
...
static int hv_gpci_init(void)
{
	...
	/* Sysinfo interface files are not available for power9 and previous platforms */
